...
impl OpError {
    ...
    pub fn reason(&self) -> Option<Reason> {
        match self.kind {
            Kind::Reset(_, reason, _)
            | Kind::GoAway(_, reason, _)
            | Kind::Reason(reason) => Some(reason),
            _ => None,
        }
    }
    ...
    pub fn is_remote(&self) -> bool {
        matches!(
            self.kind,
            Kind::GoAway(_, _, Initiator::Remote)
                | Kind::Reset(_, _, Initiator::Remote)
        )
    }
    ...
    fn from(src: ProtoError) -> OpError {
        ...
        OpError {
            kind: match src {
                ...
                Io(kind, inner) => Kind::Io(inner.map_or_else(
                    || kind.into(),
                    |inner| io::Error::new(kind, inner),
                )),
            },
        }
    }
...
impl fmt::Display for OpError {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        let debug_data = match self.kind {
            ...
            Kind::Reason(reason) => {
                return write!(fmt, "protocol error: {}", reason);
            }

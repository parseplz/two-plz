...
#[derive(Debug)]
pub struct HttpFramedRead<T> {
    inner: TokioFramedRead<T, LengthDelimitedCodec>,
    // hpack decoder state
    hpack: hpack::Decoder,
    ...
    T: AsyncRead + Unpin,
{
    ...
    pub async fn read_frame(&mut self) -> Result<Frame, ProtoError> {
        loop {
            let raw_frame = crate::io::read_frame(&mut self.inner)
                .await
                .map_err(io_err_to_proto_err)?;
            let Self {
                ref mut hpack,
                max_header_list_size,
                ref mut partial,
    ...
    pub fn set_max_frame_size(&mut self, val: usize) {
        assert!(
            DEFAULT_MAX_FRAME_SIZE as usize <= val
                && val <= MAX_MAX_FRAME_SIZE as usize
        );
        self.inner
            .decoder_mut()
            .set_max_frame_length(val);
        // Update max CONTINUATION frames too, since its based on this
        self.max_continuation_frames =
            calc_max_continuation_frames(self.max_header_list_size, val);
    }
    ...
    pub fn set_max_header_list_size(&mut self, val: usize) {
        ...
        // Update max CONTINUATION frames too, since its based on this
        self.max_continuation_frames =
            calc_max_continuation_frames(val, self.max_frame_size());
    }
    ...
    fn load_hpack(
        ...
        match *self {
            Continuable::Headers(ref mut h) => {
                h.load_hpack(src, max_header_list_size, decoder)
            }
            Continuable::PushPromise(ref mut p) => {
                p.load_hpack(src, max_header_list_size, decoder)
            }
        }
    }
}
...
fn decode_frame(
    ...
    let frame = match kind {
        ...
        Kind::WindowUpdate => {
            let res =
                frame::WindowUpdate::load(head, &bytes[frame::HEADER_LEN..]);
        ...
        Kind::Continuation => {
            ...
            if partial.buf.is_empty() {
                ...
                partial
                    .buf
                    .extend_from_slice(&bytes[frame::HEADER_LEN..]);
            }

...
macro_rules! assert_headers {
    ($frame:expr) => {{
        match $frame {
            two_plz::frame::Frame::Headers(v) => v,
...
macro_rules! assert_settings {
    ($frame:expr) => {{
        match $frame {
            two_plz::frame::Frame::Settings(v) => v,
...
macro_rules! assert_go_away {
    ($frame:expr) => {{
        match $frame {
            two_plz::frame::Frame::GoAway(v) => v,
...
#[track_caller]
pub fn assert_frame_eq<T, U>(t: T, u: U)
where
    T: Into<Frame>,
    U: Into<Frame>,
{

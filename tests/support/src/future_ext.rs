...
/// Wraps futures::future::join to ensure that the futures are only polled if
/// they are woken.
...
/// Wraps futures::future::join3 to ensure that the futures are only polled if
/// they are woken.
...
/// Wraps futures::future::join4 to ensure that the futures are only polled if
/// they are woken.
...
/// Wraps futures::future::try_join to ensure that the futures are only polled
/// if they are woken.
...
/// Wraps futures::future::select to ensure that the futures are only polled if
/// they are woken.
pub fn select<A, B>(
    future1: A,
    future2: B,
) -> futures::future::Select<Wakened<A>, Wakened<B>>
where
    A: Future + Unpin,
    B: Future + Unpin,
{
...
/// Wraps futures::future::join_all to ensure that the futures are only polled
/// if they are woken.
...
/// A future that only polls the inner future if it has been woken (after the
/// initial poll).
